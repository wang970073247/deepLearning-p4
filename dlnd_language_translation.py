...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    cut_last_one = tf.strided_slice(target_data, [0,0], [batch_size,-1],[1,1])
    go = tf.fill([batch_size, 1], target_vocab_to_int['<GO>'])
    preprocessed_input = tf.concat([go, cut_last_one], 1)
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    # TODO: Implement Function
    lstm_cell = tf.contrib.rnn.BasicLSTMCell(rnn_size)
    mul_rnn_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell] * num_layers)
    dec_cell = tf.contrib.rnn.DropoutWrapper(mul_rnn_cell, keep_prob)
